...
async fn capture_page_interfaces(
    ...
    let rendered = handlebars.render("capture_template", &context).map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?;
...
async fn main() {
    ...
    let template_content = read_to_string(path).await.expect("Cannot read template file");

    // Register the template with Handlebars
    handlebars.register_template_string("capture_template", &template_content).expect("Failed to register template");
