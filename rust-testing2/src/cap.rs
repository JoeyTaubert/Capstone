...
pub fn choose_int() -> String {
    ...
    // Record user input
    std::io::stdin()
        .read_line(&mut int_choice)
        .expect("[-]ERROR: Error, no valid interface selected");

    // Some type-casting
    let int_choice2: &str = int_choice.as_str().trim_end();

    int_choice2.to_string()
}
    ...
    loop {
        match all_interfaces.contains(&interface_input) {
            ...
            false => {
                ...
                println!(
                    "ERROR[-]: Interface '{}' not found, please try again.",
                    &interface_input
                );
...
pub async fn main() {
    ...
    loop {
        ...
        match packet_choice_string.trim().parse::<i32>() {
            ...
            Err(_) => {
                eprintln!(
                    "\n[-]ERROR: Invalid input for number of packets, '{}' not an integer.",
                    packet_choice_string.trim()
                );
            }
        };
    }
